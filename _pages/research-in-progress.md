---
title: "Research in Progress"
permalink: /research-in-progress/
author_profile: false
classes: wide
---

## Iteration 1 — September 25, 2026

## Current Result

### Primary Sources

#### Interview Data, Memos, and Notes

I conducted 7 interviews during the summer and found that 2 of these didn't quite align with the purpose of the study. The project is designed around people who are genuinely engaged in a romantic relationship with an AI companion, with the goal of understanding their perceptions of, and emotional responses to, that relationship. The excluded participants were using AI companions more as an experiment or a general-purpose companion. They reached out to me for the interview, eager to talk about the experience, in part because it is not always acknowledged or understood by people around them.

> **A methodological question here:** I certainly bring my own interpretation here — to me, an interviewee was doing "experiment" rather than "genuine romantic engagement." But the person saw my poster and decided they fit the description, which indicates that they themselves thought their experience could be considered as such. I didn't want to negate their own feeling, but their experience didn't fit the research's purpose. Is there a proper way to balance staying disciplined about the scope of my research subjects with respecting the interviewee's own perspective on their experience? What should I do with this data?

I have gone through the first round of close, immersive reading on the 5 interviews retained for the current phase of the project. This produced two kinds of notes:
- **Notes** — annotations of significant expressions and terms used by the interviewees themselves, along with my own interpretive reactions as I read.
- **Memos** — distilled points that speak back to the research question, along with a short description of each interviewee.

**Future plan:** Recruit more interviewees and begin coding (analyzing and interpreting the interview data).

**Tools:** Obsidian for organizing interview recordings, transcriptions, memos, and notes; NVivo (planned) for analysis.

#### Media Content Provided by Interviewees

I still need to find an effective method to organize this. Right now it's all stored on my phone.

### Secondary Sources

- Literature on human–AI relationships and romance (HCI, communication)
- Literature on mourning (cultural studies, affect theory)
- Literature on technological death (information studies)

**Tool:** Zotero, for organizing the literature.

### Writing Completed So Far

- I wrote a [proposal] for this project
  [Project Proposal](链接地址)

---

## Part 2: Responses to Prompt Questions

### 1. What is your hypothesis, thesis, or engaging line of inquiry that you would like to investigate during this course? Which digital tools do you plan to use in your process? Why do you think your research would be well-served by the application of these digital tools?

The project investigates people who are genuinely engaged in a romantic relationship with an AI companion, aiming to understand how they perceive that relationship and how they respond to it emotionally. Specifically, I intervene at a special moment when the relationship was discontinued due to a model upgrade. My questions are: How do Chinese users describe and respond to the loss of an AI partner? And how do their responses reflect their experiences and expectations of human–AI romance?

I use Notion for writing, Obsidian for organizing interview data, Zotero for literature, and plan to use NVivo for analysis. This is my first time using Obsidian. As an organizing tool rather than a web-based notebook, it keeps all data stored locally on my own computer, which aligns with the ethical principles on how this data should be stored and handled. It makes it easy for me to keep each interview's recording, transcript, memos, and notes organized together. To be fair, a simple folder structure on the computer could do that too. But Obsidian gives me a clearer interface for viewing these records, and more flexible multi-window support, so I can listen to the recording, look at the transcript, and take notes all at the same time. I'm also curious about its graph view feature, which can visualize links between notes, though I'm still figuring out how it might serve my research.

I had used Excel for coding in my earlier projects during my master's program, and it worked well enough. However, these interviews are more intensive (about 1.5 hours each), and it would be more effective and professional to use a specialized tool for that.

### 2. In what ways has the content assigned so far, and our discussions in class so far, impacted your thinking? How have you let these forms of engagement change your mind about your research process? Were there particular authors who grabbed your attention? What have you learned about the ways that the interpretive disciplines take advantage of the digital ecosystem of the academy?

"Proxy" was an abstract concept to me. I now think of it as a chain of interpretation. I take the words an interviewee says and use them to infer their feelings, opinions, and attitudes. Thinking about it this way has helped me form better — more specific and more answerable — questions.

I learned from a psychoanalysis course that "all you have is words." But in an interview, what I received is more than words. I perceive my interviewees' emotions. They cry and laugh during the interview. That is a subtler kind of proxy, but it's valuable and part of what makes the interview method significant and charming.

Myself as a whole person (not only as a researcher) is also a part of the chain of interpretation. I "insert" myself into their words (my data) and produce meaning from my mind, which is shaped by my lived experience and academic training. I know that all research is subjective and the interpretive paradigm just makes it most visible. However, I think that we still carry the responsibility to make our research and writing (or other forms of products) meaningful to other people. In the process of the research — from designing, forming questions, to collecting and analyzing data — I think it is critical to keep asking oneself: why should your personal interpretation of some specific individuals' words matter to anyone else? That requires balancing an honest acknowledgment of your own limitations and subjectivity with standing your ground in the way that, through research and theoretical methods, your interpretation can surface something genuinely useful for understanding humanity.

### 3. What has your Mindful Practice Journal shown you about your process thus far? Please summarize what you have written down, and go ahead and draw preliminary conclusions from your experience.

I wrote mostly about my note-taking process since that's the main thing I've been working on recently. The most significant thing I learned from the journal is a set of reflections on my interview technique.

Sometimes I'm proud of the follow-up questions I asked at a certain point because they were so right on time, in the sense that I immediately caught something in their words that needed more elaboration, or that it was the perfect timing to expand to a deeper question.

Sometimes I might also be frustrated that I missed something important. I found my interviewees were prone to using abstract words and jargon, and I took it for granted (these words have certain meanings in my head when I heard them), so I didn't ask them to explain or define them. When I looked back at the transcription, I realized that some expressions need more clarity, and some words might not mean the same thing to me and to them.

I also found that later in the interview, there's a bigger chance I miss the right time to ask a follow-up question. It's probably because I got tired and couldn't focus as well as I expected. So it might not be all good that the interview was conducted at such a "deep" length. (I'm still struggling with this: it was fascinating that they showed such passion for the topic we were talking about. However, as I've been analyzing this, I've realized that energy and focus really do affect the quality of the interview. It also makes coding a much bigger job. Should I consider limiting the interview time?)

### 4. What were your goals for this iteration and how well do you feel you achieved them? What feedback would be most useful to you right now?

My goal this iteration was to settle on a research agenda (I had been considering several possible directions), the tools I would use, and the outcomes I expect. I feel I largely achieved this. I still need more time to get familiar with NVivo before I start coding.

I would welcome feedback on:

- How to better manage the image-text data gathered from social media
- Any theories or existing studies you're aware of that might be relevant
- Your broader perspective on human-AI romantic relationships as a phenomenon
- Whether there's any background information or knowledge that would need explaining for a reader unfamiliar with this specific area
- The questions I raised about how to handle the interviews and the interview data:
  - Whether I can comfortably exclude interview data I consider irrelevant to the study's scope
  - Whether I should be limiting interview length going forward, given how much a longer interview seems to cost me in attention and follow-up quality

---
